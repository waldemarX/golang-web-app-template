...
func GetNotes(c *fiber.Ctx) error {
	...
	if len(notes) == 0 {
		return c.Status(404).JSON(fiber.Map{
			"status": "error", "message": "No notes present", "data": nil,
		})
	}
...
func DeleteNote(client *fiber.Ctx) error {
	db := database.DB
	var note model.Note

	// Read the param noteId
	id := client.Params("noteId")
	...
	// If no such note present return an error
	if note.ID == 0 {
		return client.Status(404).JSON(fiber.Map{"status": "error", "message": "No note present", "data": nil})
	}
	...
	if err != nil {
		return client.Status(404).JSON(fiber.Map{"status": "error", "message": "Failed to delete note", "data": nil})
	}

	// Return success message
	return client.JSON(fiber.Map{"status": "success", "message": "Deleted Note"})
}
